...
custom_robo_location = {}
custom_robo_location.__index = custom_robo_location
...
function custom_robo_location.new(name)
    local self = setmetatable({}, custom_robo_location)
    if name then
...
function custom_robo_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = custom_robo_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function custom_robo_location:connect_two_ways(exit, rule)
...
function custom_robo_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function custom_robo_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function custom_robo_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function custom_robo_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function custom_robo_location:accessibility()
...
-- 
function custom_robo_location:discover(accessibility, keys)
...
entry_point = custom_robo_location.new("entry_point")
